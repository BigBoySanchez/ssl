...
def main():
    ...
    assert all(s in dsd_raw for s in ["train","dev","test"]), "need train/dev/test splits"

    # Keep test IDs & golds for later (before we strip columns)
    test_ids   = dsd_raw["test"][args.id_col]
    test_golds = dsd_raw["test"][args.label_col]

    # --- Build label maps, tokenize, and trim for training ---
    labs = sorted(set(dsd_raw["train"][args.label_col]) | set(dsd_raw["dev"][args.label_col]))
    label2id = {l:i for i,l in enumerate(labs)}
    id2label = {i:l for l,i in label2id.items()}

    tok = AutoTokenizer.from_pretrained(args.model_name, use_fast=True)

    # add "labels" int column as scalar per example (not nested lists)
    # use non-batched mapping so each example's `labels` is an int (what the trainer expects)
    dsd = dsd_raw.map(lambda example: {"labels": label2id[example[args.label_col]]})
    # tokenize
    dsd = dsd.map(lambda b: tok(b[args.text_col], truncation=True), batched=True)
    ...
    train_ds, dev_ds, test_ds = map(trim, [dsd["train"], dsd["dev"], dsd["test"]])
    ...
    for lr, ep, bs in itertools.product(args.lrs, args.epochs, args.batch_sizes):
        ...
        targs = TrainingArguments(
            ...
            eval_strategy="epoch", save_strategy="yes",
    ...
    print(f"[best] cfg={best_cfg} {args.selection_metric}={best_score:.4f}")
    ...
    targs = TrainingArguments(
        output_dir=f"{args.output_dir}/best", eval_strategy="no", save_strategy="yes",
        learning_rate=lr, num_train_epochs=ep,
        per_device_train_batch_size=bs, per_device_eval_batch_size=max(8, bs),
        report_to=[], seed=args.seed
    )
    ...
    with open(os.path.join(args.output_dir, "pred_bert.csv"), "w", newline="", encoding="utf-8") as f:
        ...
        for _id, g, p in zip(test_ids, test_golds, pred_labels):
            w.writerow([_id, g, p])
